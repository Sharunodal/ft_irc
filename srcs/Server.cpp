...
/*   Updated: 2025/05/22 09:27:00 by arissane         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Server::setupServSocket(){
	// 1. Socket creation
	Logger::log(Logger::INFO, "initServer::Socket creation ");
...
void	Server::removeClient(Client& usr, std::string reason){
	...
	for (auto it = channels_.begin(); it != channels_.end(); ){
		...
		if (channel_ptr->isUserInList(usr, USERTYPE::REGULAR) || channel_ptr->isUserInList(usr, USERTYPE::INVITE)){
